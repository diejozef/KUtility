...
static std::vector<SpellInfo> g_vecPredefinedSpellDatabase = {
	...
	SpellInfo( "Yasuo", "YasuoQ3", "YasuoQ3Mis", kSlotQ, kSpellSkillshotLine, 0.3f, 1150.0f, 90.0f, 1250.0f, { "YasuoQ3W" }, { "YasuoQ3W" } ),
...
static std::vector<DashInfo> g_vecDashDatabase{
...
{
	for ( auto spell : g_vecSpellDatabase )
	{
		if ( spell.m_slot == slot && !strcmp( spell.m_strChampionName, champName ) )
		{
			if ( !_stricmp( spell.m_strSpellName, spellName ) )
			{
				out = spell;
				return true;
			}

			if ( !spell.m_vecExtraSpellNames.empty() )
			{
...
auto SpellDb::GetSpellByMissile( const char* projectileName, SpellInfo& out ) -> bool
{
	for ( auto spell : g_vecSpellDatabase )
	{
		...
		if ( !spell.m_vecExtraProjectileNames.empty() )
		{
...
{
	auto i = std::find_if( g_vecDashDatabase.begin(), g_vecDashDatabase.end(), [&]( const DashInfo& dash )
	{
		return !_stricmp( dash.m_strSpellName, spellName );
	} );

	if ( i != g_vecDashDatabase.end() )
	{
		out = *i;
		return true;
	}
...
auto SpellDb::LoadSpells() -> void
{
	...
	{
		for ( auto spell : g_vecPredefinedSpellDatabase )
		{
