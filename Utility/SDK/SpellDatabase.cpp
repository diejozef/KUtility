...
static std::vector<SpellInfo> g_vecPredefinedSpellDatabase = {
	...
	SpellInfo("Ashe", "EnchantedCrystalArrow", "EnchantedCrystalArrow", kSlotR, kSpellSkillshotLine, 0.25f, 25000.0f, 130.0f, 1600.0f, { }, { }),
	...
	SpellInfo("Ezreal", "EzrealTrueshotBarrage", "EzrealTrueshotBarrage", kSlotR, kSpellSkillshotLine, 1.0f, 25000.0f, 160.0f, 2000.0f, { }, { }),
	...
	SpellInfo("Jinx", "JinxR", "JinxR", kSlotR, kSpellSkillshotLine, 0.6f, 25000.0f, 140.0f, 1700.0f, { "JinxRWrapper" }, { "JinxRWrapper", "JinxRMissile" }),
...
/*
use map for basic spell names(fast access)
use vector when an extra spell/projectile name gets passed or
the spell name is messed up case-wise so
it wasn't found in the map and _stricmp is needed
*/

std::vector<SpellInfo> g_vecSpellDatabase;
std::unordered_map<std::string, SpellInfo> g_mapSpellDatabase;
std::unordered_map<std::string, SpellInfo> g_mapMissileDatabase;
std::unordered_map<std::string, DashInfo> g_mapDashDatabase;

static std::vector<SmiteInfo> g_vecSmiteDatabase{
...
{
	if (g_mapSpellDatabase.find(std::string(spellName)) != g_mapSpellDatabase.end())
	{
		out = g_mapSpellDatabase[std::string(spellName)];
		return true;
	}
	else
	{
		...
		{
			if (spell.m_slot == slot
				&& !strcmp(spell.m_strChampionName, champName)
...
{
	if (g_mapMissileDatabase.find(std::string(projectileName)) != g_mapMissileDatabase.end())
	{
		out = g_mapMissileDatabase[std::string(projectileName)];
		return true;
	}
	else
	{
...
{
	if (g_mapDashDatabase.find(std::string(spellName)) != g_mapDashDatabase.end())
	{
		out = g_mapDashDatabase[std::string(spellName)];
		return true;
	}
...
{
	auto i = std::find_if(g_vecSmiteDatabase.begin(), g_vecSmiteDatabase.end(), [&](const SmiteInfo& smite)
	{
		return !strcmp(smite.m_strChampionName, championName);
	});

	if (i != g_vecSmiteDatabase.end())
	{
		out = *i;
		return true;
	}
	...
	for (auto hero : GEntityList->GetAllHeros(false, true))
	{
		for (const auto& spell : g_vecPredefinedSpellDatabase)
		{
			if (!strcmp(spell.m_strChampionName, hero->ChampionName()))
			{
				...
				g_mapSpellDatabase[spell.m_strSpellName] = spell;
				g_mapMissileDatabase[spell.m_strProjectileName] = spell;
			}
		}
	}

	g_mapDashDatabase.emplace(std::make_pair(std::string("SummonerFlash"), DashInfo("SummonerFlash", 425.0f)));
	g_mapDashDatabase.emplace(std::make_pair(std::string("Deceive"), DashInfo("Deceive", 400.0f)));
	g_mapDashDatabase.emplace(std::make_pair(std::string("EzrealArcaneShift"), DashInfo("EzrealArcaneShift", 475.0f)));
}